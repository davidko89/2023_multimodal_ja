...
from code.src.utility_rgbd import rotation_matrix_to_euler_angles, camera_matrix, detector, get_image_points_and_model_points, draw_face_bounding_boxes, write_headpose_to_csv 
...
processed_participants_file = PROC_PARTICIPANT_PATH.joinpath("processed_participants1.txt")
...
headpose_csv_file = Path(HEADPOSE_PATH, "headpose_values1.csv")

# Check if the file exists. If not, write the header.
if not os.path.exists(headpose_csv_file):
    with open(headpose_csv_file, 'w') as f:
        f.write('participant_id,roll,pitch,yaw\n')
    ...
    if depth_file.exists():
        ...
        for frame_idx, (depth_image, color_image) in enumerate(zip(depth_images, color_images)):
            ...
            for face in faces:
                ...
                roll, pitch, yaw = rotation_matrix_to_euler_angles(matrix)

                print("Rotation Matrix:\n {0}".format(matrix))
                print("Euler Angles:\n {0}".format([roll, pitch, yaw])) 

                write_headpose_to_csv(str(Path(HEADPOSE_PATH, "headpose_values1.csv")), participant_id, roll, pitch, yaw)
