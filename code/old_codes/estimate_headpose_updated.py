...
from code.src.utility_sample import rotation_vector_to_euler_angles, camera_matrix, detector, get_image_points_and_model_points, draw_face_bounding_boxes, write_headpose_to_csv 
...
if not os.path.exists(headpose_csv_file):
    with open(headpose_csv_file, 'w') as f:
        f.write('participant_id,pitch,yaw,roll\n')
    ...
    if depth_file.exists():
        ...
        for frame_idx, (depth_image, color_image) in enumerate(zip(depth_images, color_images)):
            ...
            for face in faces:
                ...
                write_headpose_to_csv(str(Path(HEADPOSE_PATH, "headpose_values.csv")), participant_id, pitch, yaw, roll)
